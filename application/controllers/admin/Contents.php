...
class Contents extends Admin_Controller
{
	...
	function __construct()
	{
        ...
        {
            $this->session->set_flashdata('message','You are not allowed to visit the Contents page');
    ...
    public function create($content_type = 'page', $language_slug = NULL, $content_id = 0)
    {
        ...
        {
            $this->session->set_flashdata('message', 'A translation for that content already exists.');
    ...
    public function edit($language_slug, $content_id)
    {
        ...
        if($content == FALSE)
        {
            $this->session->set_flashdata('message', 'There is no content to translate.');
        ...
        {
            $this->session->set_flashdata('message', 'There is no translation for that content.');
            redirect('admin/contents/index/'.$content_type, 'refresh');
        }
    ...
    public function publish($content_id, $published)
    {
        ...
        if( ($content != FALSE) && ($published==1 || $published==0))
        {
            ...
            {
                $this->session->set_flashdata('message', 'The published status was set.');
            }
            else
            {
                $this->session->set_flashdata('message', 'Couldn\'t set the published status.');
            }
        }
        else
        {
            $this->session->set_flashdata('message', 'Can\'t find the content or the published status isn\'t correctly set.');
        }
        ...
        if($content = $this->content_model->get($content_id))
        {
            if($language_slug=='all')
            {
                if($deleted_translations = $this->content_translation_model->where('content_id',$content_id)->delete())
                {
                    ...
                    $this->session->set_flashdata('message', $deleted_pages.' page deleted. There were also '.$deleted_translations.' translations, '.$deleted_keywords.' keywords, '.$deleted_keyphrases.' key phrases, '.$deleted_slugs.' slugs and '.$deleted_images.' images deleted.');
                }
                else
                {
                    $deleted_pages = $this->content_model->delete($content_id);
            ...
            else
            {
                if($this->content_translation_model->where(array('content_id'=>$content_id,'language_slug'=>$language_slug))->delete())
                {
                    ...
                    $this->session->set_flashdata('message', 'The translation, '.$deleted_keywords.' keywords, '.$deleted_keyphrases.' key phrases and '.$deleted_slugs.' slugs were deleted.');
                }
            }
        }
        else
